...
def main():
    ...
    parser.add_argument('--max_fonts', type=int, default=50, help='사용할 폰트 최대 개수(기본 50)')
    ...
    # 최대 사용 폰트 수 제한 (seed로 셔플 후 상위 N개)
    if args.max_fonts and len(font_paths) > args.max_fonts:
        rng = random.Random(args.seed)
        rng.shuffle(font_paths)
        font_paths = font_paths[:args.max_fonts]
    print(f'사용할 폰트 파일: {len(font_paths)}개 (max_fonts={args.max_fonts})')
